...
class validacoes {
    ...
    protected function set_messages_default(){
        $this->_messages = array(
            ...
            'is_float'       => 'O valor %s não é válido ',
...
class validacoes{
    ...
    private function minlenght($param, $valor){
        $pattern = '/.{'.$param.',}/';
        return preg_match($pattern, $valor);
    }
    
    private function maxlenght($param, $valor){
	...
	private function psnValida($param, $valor){
		$pattern = '/(^[\w-]{3,16})$/';
		return preg_match($pattern, $valor);
	}
    
	private function emailValido($param, $valor){  
        if($param){ //TRUE
            $regex = '/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,3})$/'; 
            return preg_match($regex, $valor);
        }    
    }
	
	private function senhaValida($param, $valor){    
        $pattern = '/(^[\w-!#@+]{6,10})$/';
		return preg_match($pattern, $valor);  
    }
	
	private function min($param, $valor){
        if ($valor < $param) {
            return 0;
        }
        return 1;
    ...
    private function max($param, $valor){
        if ($valor > $param) {
            return 0;
        }
        return 1;
	...
	private function int($param, $valor){
    	if ($valor == '') //se o campo n�o for requerido
			return 1;
        $pattern = '/^[0-9]{1,6}$/';
        if(is_numeric($valor)){ // � n�mero
            if (preg_match($pattern, $valor)){ // � inteiro
                return 1;
            }        
        }
        return 0;
    ...
    private function numPares($param, $valor){
    	if($param){ //TRUE
	        if($valor%2 > 0){ //número ímpar = falha
	            return 0; 
	        }
		}
        return 1;
    ...
    private function data($param, $valor){
    	if ($valor == '') //se o campo n�o for requerido
			return 1;
        
        $pattern = '/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/';
        return preg_match($pattern, $valor); // se data = 1, sen�o 0
    }
    

	private function selecionaMensagem($prop, $campo, $val){
		switch ($prop) {
			case 'requerido':
				$msg = ": Preenchimento Obrigatório!";
				break;
			case 'minlenght':
				$msg = ": Quantidade de caracteres mínimos = ".$val;
				break;
            case 'maxlenght':
                $msg = ": Quantidade de caracteres máximos = ".$val;
                break;
			case 'psnValida':
				$msg = ": A PSN ID deve ter entre 3 e 16 caracteres, podendo conter letras, números, hífens(-) ou sublinados(_).";
				break;
			case 'senhaValida':
				$msg = ": A senha deve ter entre 6 e 10 caracteres, podendo conter letras, números e alguns caracteres especiais.";
				break;
			case 'int':
				$msg = ": Valor precisa ser um número inteiro e positivo!";
				break;
			case 'data':
				$msg = ": Formato de Data Inválido!";
				break;
			case 'min':
				$msg = ": Valor Mínimo = ".$val;
				break;
			case 'max':
				$msg = ": Valor Máximo = ".$val;
				break;
            case 'emailValido':
                $msg = ": E-mail inválido!";
                break;
            case 'numPares':
                $msg = ": Quantidade de Participantes precisa ser um número PAR!";
                break;
			default:
				$msg = ": Erro inesperado";
				break;
		...
		return $msg = strtoupper($campo).$msg;

	}

    public function validaData($dt){
        if (strstr($dt, "/")){//verifica se tem a barra /
           $d = explode ("/", $dt);//tira a barra
           $dt = "$d[2]-$d[1]-$d[0]";//separa as datas $d[2] = ano $d[1] = mes etc...
        } else{
             if (strstr($dt, "%2F")){
               $d = explode ("%2F", $dt);
               $dt = "$d[2]-$d[1]-$d[0]";
